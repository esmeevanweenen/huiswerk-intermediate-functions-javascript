...
function cumLaude(arrayWithNumbers) {
    let count = 0;

    for (let i = 0; i < arrayWithNumbers.length; i++) {
        if (arrayWithNumbers[i] >= 8) {
            count = count + 1;
        }
...
const firstList = cumLaude(grades);
const secondList = cumLaude([6, 4, 5]);
const thirdList = cumLaude([8, 9, 4, 6, 10]);
